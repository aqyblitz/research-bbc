...
int main(int argc, char *argv[])
{
    ...
    uint32_t msg_info[] = {htonl(sizeof(int)), htonl(0), htonl(4), htonl(6)};

    unsigned char* temp = reinterpret_cast<unsigned char*>(msg_info);
    crcgen gen = crcgen();
    uint32_t crc_val = gen.crc_compute(temp, sizeof(temp));
    printf("The computed CRC is 0x%X\n", crc_val);

    // TODO: There's a way to do this with vectors in C++ that's clever. Dunno about .js; it leverages the contiguous storage of vector elems. Skip for now.
    uint32_t msg_info_send[] = {htonl(sizeof(int)), htonl(0), htonl(4), htonl(6), crc_val};

    bzero(buffer,256);
    n = write(sockfd, (char*) msg_info_send, sizeof(msg_info_send));
