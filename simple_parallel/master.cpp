...
int main(int argc , char *argv[])
{
    ...
    int master_socket , addrlen , new_socket , client_socket[block_total*red_mult] , max_clients = block_total*red_mult , activity, i , valread , sd;
    int max_sd;
    ...
    while(slave_count < block_total)
    {
        ...
        if (FD_ISSET(master_socket, &readfds))
        {
            ...
            if(slave_count == vertex_total && vertex_total%block_total>0)
            {
                ...
                if( send(new_socket, sizes, sizeof(int32_t)*2, 0) != sizeof(int32_t)*2 )
                {
                    perror("send");
                }
                if( send(new_socket, &adj_matrix[(slave_count-1)*vertex_total/block_total], send_size, 0) != send_size )
                {
                    perror("send");
                }
            }
    ...
    // Assign IDs to slaves & blocks of data.
